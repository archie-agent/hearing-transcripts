...
def main():
    ...
    try:
        ...
        if args.discover_only:
            ...
            queue_status = "discover_only"
            return

        if not new_hearings:
            log.info("All hearings already processed.")
            queue_status = "noop_already_processed"
            return
        ...
        if queue_write_enabled:
            state.record_queue_run_finish(
                run_id=run_id,
                status=queue_status,
                hearings_discovered=queue_discovered,
                hearings_processed=queue_processed,
                hearings_failed=queue_failed,
                error=queue_error,
            )
