...
def main():
    ...
    try:
        ...
        for winner, loser, committee_key, date in merges:
            ...
            conn.execute("DELETE FROM processing_steps WHERE hearing_id = ?", (loser_id,))
            conn.execute("DELETE FROM cspan_title_searches WHERE hearing_id = ?", (loser_id,))
            conn.execute("DELETE FROM hearings WHERE id = ?", (loser_id,))
